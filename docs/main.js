...
DoubanApi.prototype.make_api_url = function(
  type,
  user,
  key,
  status,
...
DoubanApi.prototype.show = function() {
  ...
  $.each(books, function(i, item) {
    $.each(item.indexs, function(t, idx) {
      tmpthis.appendScript(
        tmpthis.all_url("book", item.status, idx.begin, idx.end)
      );
    });
  });
};
  ...
  if (!this[type + status + "_show"]) {
    this[type + status + "_show"] = function(json) {
      if ($("#" + type + status).length === 0) {
        ...
        $(
          '<div id="' + type + status + '" class="douban-list"><ul></ul></div>'
        ).appendTo(wrap);
        $('<div class="clear"></div>').appendTo(wrap);
      }
      $("#" + type + status + " > ul").append(
        this.make_list_item(this.parse_json(json))
      );
    };
  }
  return this.make_api_url(
    type,
    this.defaults.user,
    this.defaults.api,
    status,
